namespace IcerDesign.CCHelper
{
    ...
    internal class ClearCommands
    {
        ...
        {
            var lst = files
                .Select(file => $@"checkin -ide -c ""{comment}"" ""{file}""")
        ...
        {
            var lst = files
                .Select(file => $@"mklabel -replace ""{label}"" ""{file}""")
        ...
        public static ClearCommand[] AddToSourceControl(string path, out string[] elements, out int totalNumber, int elementLimit = 10)
        {
            ...
            else if (Directory.Exists(path))
            {
                ...
                foreach (var dir in directories)
                {
                    ...
                    commands.Add(new ClearCommand($@"mkelem -mkpath -master -nc ""{di.Name}""", di.Parent.FullName));
                    if (eles.Count < elementLimit) eles.Add(Path.Combine(di.Parent.FullName, di.Name));
                    totalNumber++;
                    var files = Directory.GetFiles(dir, "*", SearchOption.TopDirectoryOnly);
                    foreach (var file in files)
                    {
                        commands.Add(new ClearCommand($@"mkelem -mkpath -master -nc ""{file}""", dir));
        ...
        public static ClearCommand[] RemoveFromSourceControl(string path)
        {
            ...
            commands.Add(new ClearCommand($@"rmname ""{ret.Element}""", ret.Parent, (ex) =>
            {
